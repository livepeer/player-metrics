...
function reportGenericVideoMetrics(video,reportingWebsocketUrl) {
  ...
  var stats = { //save playback metrics here
    d: { //this contains the most recent data, any changes are immediately applied here
      nWaiting: 0,
      timeWaiting: 0,
      nStalled: 0,
      timeStalled: 0,
  ...
  var firstplay = function(){
    stats.d["firstPlayback"] = new Date().getTime() - reportGenericVideoMetrics_bootMs;
  ...
  var monitor = {
    ...
    averagingSteps: 20,
    init: function(){
      if (this.active) { return; } //it's already running, don't bother
    ...
    destroy: function(){
      if (!this.active) { return; } //it's not running, don't bother
